...
class ClientController extends Controller
{
    public function index(Builder $builder) {
        if (request()->ajax()) {
            return $user=DataTables::of(User::where('role_id', 3)->get())
            ->editColumn('name', function($user) {
                return $user->title . ' ' . $user->name;
            })
            ->addColumn('action',function($user){
                return '<a href="'. route('client.edit',['id'=> $user->id]) .'" class="btn btn-icon btn-primary">Edit</a>
        ...
        return view('client.index',compact('html'));
    }
    ...
    public function store(Request $request) {
        try {
            ...
            return Redirect::to('/client')->with($notification);
        } catch (\Throwable $th) {
            $notification = array(
                'message' => 'Sorry can`t add Client!',
                'alert-type' => 'error'
            );
    ...
    public function update(Request $request, $id) {
        try {
            ...
            $notification = array(
                'message' => 'Sorry can`t update client!',
                'alert-type' => 'error'
            );
